...
DROP FUNCTION IF EXISTS public.match_taxa_sources(text, text, text);
...
BEGIN
    ...
    FOR taxa_obs_record IN SELECT * FROM public.taxa_obs LOOP
        ...
        END;
    END LOOP;
    PERFORM taxa_ref_fix_synonyms();
    PERFORM fix_missing_source_parent();
END;
$BODY$;

ALTER FUNCTION public.refresh_taxa_ref()
    OWNER TO coleo;
...
BEGIN
    ...
    FOR taxa_ref_record IN 
        ...
        JOIN taxa_obs_ref_lookup ref_lu ON subset_taxa_obs.id = ref_lu.id_taxa_obs
        JOIN taxa_ref ON ref_lu.id_taxa_ref = taxa_ref.id
...
ALTER FUNCTION public.refresh_taxa_partial()
    OWNER TO coleo;

-- DROP FUNCTION IF EXISTS public.taxa_ref_fix_synonyms();
CREATE OR REPLACE FUNCTION public.taxa_ref_fix_synonyms(
	)
    RETURNS void
    LANGUAGE 'plpgsql'
AS
...
FROM taxa_ref cdpnq_ref
JOIN taxa_obs_ref_lookup cdpnq_lu ON cdpnq_ref.id = cdpnq_lu.id_taxa_ref
JOIN taxa_obs_ref_lookup gbif_lu ON cdpnq_lu.id_taxa_obs = gbif_lu.id_taxa_obs
-- taxa_ref join to filter only GBIF Backbone Taxonomy sources
JOIN taxa_ref gbif_ref ON gbif_lu.id_taxa_ref = gbif_ref.id
    AND gbif_ref.source_name = 'GBIF Backbone Taxonomy'
    AND cdpnq_ref.rank = gbif_ref.rank
JOIN taxa_obs_ref_lookup synonym_obs_lu ON gbif_ref.id = synonym_obs_lu.id_taxa_ref
...
DELETE FROM taxa_obs_ref_lookup
WHERE (id_taxa_obs, id_taxa_ref) IN (
    SELECT id_taxa_obs, id_taxa_ref
    FROM taxa_obs_ref_cdpnq_synonym_fix_lookup
);

INSERT INTO taxa_obs_ref_lookup (id_taxa_obs, id_taxa_ref, id_taxa_ref_valid, match_type, is_parent)
SELECT
    id_taxa_obs,
    id_taxa_ref,
    id_taxa_ref_valid,
...
ALTER FUNCTION public.taxa_ref_fix_synonyms()
    OWNER TO coleo;


CREATE OR REPLACE FUNCTION public.fix_missing_source_parent()
RETURNS void
LANGUAGE 'plpgsql'
...
FROM taxa_obs_ref_lookup cur_parent_lu
JOIN taxa_ref cur_parent_ref ON cur_parent_lu.id_taxa_ref = cur_parent_ref.id
JOIN taxa_ref new_parent_ref USING (scientific_name, rank)
WHERE (cur_parent_lu.id_taxa_obs, new_parent_ref.id) NOT IN (SELECT id_taxa_obs, id_taxa_ref FROM taxa_obs_ref_lookup)
...
ALTER FUNCTION public.fix_missing_source_parent()
    OWNER TO coleo;