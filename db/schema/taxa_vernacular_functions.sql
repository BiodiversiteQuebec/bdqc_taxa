...
AS $BODY$
    ...
    WITH src AS (
        ...
        RETURNING id, source_name, source_record_id, name, language
    ), vern AS (
        SELECT id as id_taxa_vernacular FROM vern_insert
        UNION ALL
        SELECT v.id
        FROM rubus.taxa_vernacular v
        JOIN src
          ON v.source_name = src.source
         AND v.source_record_id = src.source_taxon_key
         AND v.name = src.name
         AND v.language = src.language
    ), links AS (
        SELECT
            ref_id,
            v.id_taxa_vernacular
        FROM unnest(id_taxa_ref) AS ref_id
        CROSS JOIN vern v
    )
    ...
    SELECT ref_id, id_taxa_vernacular
    FROM links
    ON CONFLICT DO NOTHING;
